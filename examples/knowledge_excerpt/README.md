# llama.cpp/example/knowledge_excerpt

Given question and the knowledge paragraph, this tool can excerpt the exact content inside the knowledge text to address the question
...
```bash
./kne -m models/7B/ggml-model.bin --prompt "Once upon a time" --knowleget-str "knowledge content" 
```
or

```bash
./kne -m models/7B/ggml-model.bin --prompt "Once upon a time" --knowleget-file "knowledge content path" 
```
